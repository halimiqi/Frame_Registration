...
class  FullConnect(object):
    ...
    def BuildFullConnect(self):
        ...
        with tf.variable_scope("network"):
            ...
            self.linear5, self.w["w5_1"], self.b["b5_1"] = linear(self.linear4, 2048, activation_fn=activation_fn,
                                                                  name='linear5')
            self.linear6, self.w["w6_1"], self.b["b6_1"] = linear(self.linear5, 6, activation_fn=activation_fn, name = 'linear6')
            ####change the loss
            self.pred  = tf.multiply(self.linear6,self.std) + self.mean
            self.y_out = tf.multiply(self.Y, self.std) + self.mean
            self.delta = tf.subtract(self.pred, self.y_out)
            self.small_delta = tf.subtract(self.linear6,self.Y)
            ##L2 normalization
    ...
    def Train(self,trainX, trainY,validX, validY,mean, std):
        ...
        for i in tqdm(range(self.step_op.eval(),self.Config.MaxTrainStep)):
            ...
            if i% self.Config.TestStep == self.Config.TestStep-1:
                ...
                strvalidlist = self.Sess.run(self.summary_acc_op,feed_dict ={self.X:validX[validBatchSize*(self.Config.TestNum-1):len(validX),...], self.Y:validY[validBatchSize*(self.Config.TestNum-1):len(validX),...]} )
    ...
    def Play(self,x,y,mean,std):
        print("!!!!!start Play!!!!!")
        self.load_model()
        pred = self.linear6.eval({self.X:x, self.Y:y})
        pred_out = pred * std + mean
        y_out = y*std + mean
        acc = self.acc.eval({self.X:x,self.Y:y})
